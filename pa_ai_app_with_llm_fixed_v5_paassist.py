...
# ----------------- Tab 6: ค้นหาข้อตรวจพบที่ผ่านมา (The code you provided) -----------------
with tab_issue:
...
# ----------------- Tab 7: สรุปข้อมูล (Preview) (Completed code) -----------------
with tab_preview:
...
"""
    ...
    if not logic_df.empty:
        st.dataframe(logic_df[["type", "description", "metric", "target", "source"]], use_container_width=True, hide_index=True)
    else:
        st.info("ยังไม่มีรายการ Logic Model")

    st.markdown("### Methods (วิธีเก็บข้อมูล)")
    if not methods_df.empty:
        st.dataframe(methods_df[["type", "tool_ref", "sampling", "questions", "linked_issue", "data_source", "frequency"]], use_container_width=True, hide_index=True)
    else:
        st.info("ยังไม่มีรายการ Methods")

    st.markdown("### KPIs")
    if not kpis_df.empty:
        st.dataframe(kpis_df[["level", "name", "formula", "unit", "target", "data_source"]], use_container_width=True, hide_index=True)
    else:
        st.info("ยังไม่มีรายการ KPIs")

    st.markdown("### Risks")
    if not risks_df.empty:
        st.dataframe(risks_df[["description", "category", "likelihood", "impact", "mitigation", "hypothesis"]], use_container_width=True, hide_index=True)
    else:
        st.info("ยังไม่มีรายการ Risks")

    st.markdown("### ประเด็นการตรวจสอบที่เลือก")
    if not audit_issues_df.empty:
        st.dataframe(audit_issues_df[["title", "rationale", "linked_kpi", "proposed_methods", "source_finding_id"]], use_container_width=True, hide_index=True)
    else:
        st.info("ยังไม่มีประเด็นการตรวจสอบที่เลือกจากรายงานเก่า")

    st.divider()

    # NOTE: The create_excel_template function only creates an empty template. 
    # If you intend to export the whole plan data, you will need to implement a function for that.
    st.download_button(
        label="⬇️ ดาวน์โหลดแผนงาน (Excel)",
        data=create_excel_template(), 
        file_name=f"PlanningStudio_Plan_{plan['plan_id']}.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
...
# ----------------- Tab 8: AI Assist -----------------
with tab_assist:
    st.subheader("🤖 PA Assist - แนะนำแผนงานด้วย AI")
    st.info("หน้านี้จะช่วยสรุปประเด็น, ข้อตรวจพบที่คาดว่าจะพบ, และสรุปผลรายงานเบื้องต้นจากข้อมูล 6W2H, Logic Model, และ Risks ที่คุณกรอก")
    
    # Check for API Key
    api_key_assist = st.text_input("กรุณากรอก API Key เพื่อใช้บริการ AI Assist:", type="password", key="api_key_assist")
    
    # ------------------ Prepare Prompt Data ------------------
    # Combine all planning data into a comprehensive prompt string
    
    # 1. Plan Details
    plan_details = f"""
    ## 1. รายละเอียดแผนงาน (6W2H)
    - ชื่อแผน/เรื่อง: {plan['plan_title']}
    - โครงการ/แผนงาน: {plan['program_name']}
    - Who (ใครทำ): {plan['who']}
    - Whom (เพื่อใคร): {plan['whom']}
    - What (ทำอะไร): {plan['what']}
    - Where (ที่ไหน): {plan['where']}
    - When (เมื่อใด): {plan['when']}
    - Why (ทำไม): {plan['why']}
    - How (อย่างไร): {plan['how']}
    - How much (เท่าไร): {plan['how_much']}
    ...
    # 2. Logic Model
    logic_model_str = "## 2. Logic Model\n"
    if not logic_df.empty:
        for idx, row in logic_df.iterrows():
            logic_model_str += f"- **{row['type']}**: {row['description']} ({row['metric']} เป้าหมาย: {row['target']})\n"
    else:
        logic_model_str += "ไม่พบข้อมูล Logic Model\n"
        
    # 3. Risks
    risks_str = "## 3. ความเสี่ยง (Risks)\n"
    if not risks_df.empty:
        for idx, row in risks_df.iterrows():
            risks_str += f"- **ความเสี่ยง**: {row['description']} (โอกาสเกิด: {row['likelihood']}, ผลกระทบ: {row['impact']}, สมมุติฐานที่ต้องทดสอบ: {row['hypothesis']})\n"
    else:
        risks_str += "ไม่พบข้อมูลความเสี่ยง\n"
        
    full_plan_data = plan_details + logic_model_str + risks_str
    
    # ------------------ AI Logic ------------------
    
    if st.button("🌟 ให้ AI แนะนำประเด็นการตรวจสอบ", type="primary", key="ai_assist_button"):
        if not api_key_assist:
            st.error("กรุณากรอก API Key ก่อนใช้งาน")
        else:
            with st.spinner("AI กำลังวิเคราะห์แผนงานและสร้างคำแนะนำ..."):
                try:
                    client = OpenAI(
                    ...
                    # --- Task 1: Generate Issues ---
                    issue_prompt = f"""
จากข้อมูลแผนงานด้านล่างนี้ ซึ่งเป็นแผนการตรวจสอบผลการดำเนินงาน (Performance Audit) ของคุณ
{full_plan_data}

ในฐานะผู้เชี่ยวชาญการตรวจสอบผลการดำเนินงาน โปรดวิเคราะห์ข้อมูลนี้:
1.  **ประเด็นการตรวจสอบที่ควรให้ความสำคัญ (Focus Audit Issues):** เน้นที่ประเด็นที่เกี่ยวข้องกับการวัดผล (KPIs), ความเสี่ยง (Risks), และ Output/Outcome ใน Logic Model
2.  **ระบุข้อตรวจพบที่คาดว่าจะพบ (Potential Findings) พร้อมระดับโอกาส (Likelihood):** ให้พิจารณาว่าความเสี่ยงและช่องว่างของแผนงานอาจนำไปสู่ข้อตรวจพบใดได้บ้าง และให้ระดับโอกาส (สูง/กลาง/ต่ำ)
3.  **สรุปโครงร่างรายงานเบื้องต้น (Draft Report Summary):** สรุปว่าคุณจะเขียนรายงานว่าอย่างไร โดยเน้นที่ประเด็นหลักและข้อเสนอแนะเบื้องต้น

โปรดตอบในรูปแบบ Markdown 3 ส่วน โดยมีหัวข้อดังต่อไปนี้เท่านั้น (ไม่ต้องมีข้อความนำหรือสรุป):
...
"""
                    ...
                    gen_issues = "ไม่พบข้อมูล"
                    gen_findings = "ไม่พบข้อมูล"
                    gen_report = "ไม่พบข้อมูล"
                    
                    for part in parts:
                        if "ประเด็นการตรวจสอบที่ควรให้ความสำคัญ" in part:
                            gen_issues = part.replace("ประเด็นการตรวจสอบที่ควรให้ความสำคัญ", "").strip()
                        elif "ข้อตรวจพบที่คาดว่าจะพบ" in part:
                            gen_findings = part.replace("ข้อตรวจพบที่คาดว่าจะพบ (พร้อมระดับโอกาส)", "").strip()
                        elif "สรุปโครงร่างรายงานเบื้องต้น" in part:
                            gen_report = part.replace("สรุปโครงร่างรายงานเบื้องต้น", "").strip()

                    st.session_state["gen_issues"] = gen_issues
                    st.session_state["gen_findings"] = gen_findings
                    st.session_state["gen_report"] = gen_report
                    st.success("AI วิเคราะห์แผนงานและสร้างคำแนะนำเรียบร้อยแล้ว! ✅")
                    st.balloons()
                    
                except Exception as e:
                    error_type = type(e).__name__
                    if error_type == "AuthenticationError":
                         error_message = "เกิดข้อผิดพลาดในการเชื่อมต่อ API: (AuthenticationError) โปรดตรวจสอบ API Key ของคุณ"
                    elif error_type == "RateLimitError":
                         error_message = "เกิดข้อผิดพลาดในการเชื่อมต่อ API: (RateLimitError) โปรดตรวจสอบขีดจำกัดการใช้งาน (Rate Limit) ของคุณ"
    ...
    st.markdown("<h4 style='color:#007bff;'>ประเด็นการตรวจสอบที่ควรให้ความสำคัญ</h4>", unsafe_allow_html=True)
    ...
    st.markdown("<h4 style='color:#007bff;'>ข้อตรวจพบที่คาดว่าจะพบ (พร้อมระดับโอกาส)</h4>", unsafe_allow_html=True)
    ...
    st.markdown("<h4 style='color:#007bff;'>สรุปโครงร่างรายงานเบื้องต้น</h4>", unsafe_allow_html=True)
